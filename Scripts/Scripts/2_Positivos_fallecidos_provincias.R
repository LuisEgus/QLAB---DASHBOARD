...
#*******************************************************************************
# MANEJO DE BASES DE POSITIVOS Y FALLECIDOS ORIGINALES PARA LA CREACI?N DE UNA BASE PANEL A NIVEL PROVINCIAL
#*******************************************************************************
...
#Nos quedamos solo con los 4 primeros dígitos del ubigeo (DEPARTAMENTO + PROVINCIA)
...
df_positivos$UBIGEO <-  substr(df_positivos$UBIGEO, start = 1, stop = 4)


df_fallecidos$UBIGEO <- sprintf("%06d", df_fallecidos$UBIGEO)
df_fallecidos$UBIGEO <-  substr(df_fallecidos$UBIGEO, start = 1, stop = 4)
...
# Las columnas de DEPARTAMENTO, PROVINCIA no tienen NA
...
write.csv(df_positivos,'positivos_prov_.csv')
write.csv(df_fallecidos,'fallecidos_prov_.csv')